...
DEFAULTS = {
    ...
    "trestbps": 0,
    "chol": 0,
    "fbs": "Choose an Option",
    "restecg": "Choose an Option",
    "thalach": 0,
    "exang": "Choose an Option",
    "oldpeak": 0.0,
    "slope": "Choose an Option",
    "ca": 0,
    "thal": "Choose an Option",
    "prediction": None,
    "risk_level": None
}
...
st.session_state.age = st.number_input(
    "Age", min_value=0, max_value=120, value=st.session_state.age or 0
)
...
st.session_state.trestbps = st.number_input(
    "Resting Blood Pressure (mm Hg)", min_value=0, max_value=250, value=st.session_state.trestbps or 0
)

st.session_state.chol = st.number_input(
    "Cholesterol (mg/dL)", min_value=0, max_value=600, value=st.session_state.chol or 0
)
...
st.session_state.thalach = st.number_input(
    "Maximum Heart Rate Achieved", min_value=0, max_value=250, value=st.session_state.thalach or 0
)
...
st.session_state.oldpeak = st.number_input(
    "ST Depression Induced by Exercise", min_value=0.0, max_value=10.0, value=st.session_state.oldpeak or 0.0
)
...
st.session_state.ca = st.number_input(
    "Number of Major Vessels Colored by Fluoroscopy", min_value=0, max_value=4, value=st.session_state.ca or 0
)
...
if st.button("Predict"):
    ...
    if (
       ...
       # Make prediction
        ...
        # Calculate averages from the dataset
        average_values = df.mean(numeric_only=True)
        # Prepare the "Your Input" list
        your_input = [
            st.session_state.age,
            sex_options.get(st.session_state.sex.split(" ")[0], None) if st.session_state.sex != "Choose an Option" else None,
            cp_options.get(st.session_state.cp.split(" ")[0], None) if st.session_state.cp != "Choose an Option" else None,
            st.session_state.trestbps,
            st.session_state.chol,
            fbs_options.get(st.session_state.fbs.split(" ")[0], None) if st.session_state.fbs != "Choose an Option" else None,
            restecg_options.get(st.session_state.restecg.split(" ")[0], None) if st.session_state.restecg != "Choose an Option" else None,
            st.session_state.thalach,
            exang_options.get(st.session_state.exang.split(" ")[0], None) if st.session_state.exang != "Choose an Option" else None,
            st.session_state.oldpeak,
            slope_options.get(st.session_state.slope.split(" ")[0], None) if st.session_state.slope != "Choose an Option" else None,
            st.session_state.ca,
            thal_options.get(st.session_state.thal.split(" ")[0], None) if st.session_state.thal != "Choose an Option" else None
        ]
        # Filter out rows where "Your Input" is None
        valid_indices = [i for i, val in enumerate(your_input) if val is not None]
        filtered_your_input = [your_input[i] for i in valid_indices]
        filtered_averages = average_values.iloc[valid_indices]
        # Create the comparison DataFrame
        comparison = pd.DataFrame({
            "Your Input": filtered_your_input,
            "Average": filtered_averages
        })
